...
<script type="text/javascript">
	...
	function searchForRupee(obj){
		var rootCategory = $("#rootCategory").val();
		var subRootCategory = $("#subRootCategory").val();
		var childCategory = $("#childCategory").val();
		var categoryName = $("#categoryName").val();
		$("#searchRootCategory").val(rootCategory);
		$("#searchSubRootCategory").val(subRootCategory);
		$("#searchChildCategory").val(childCategory);
		$("#searchCategoryName").val(categoryName);
				...
				<div class="uiv2-shopping-list-wrapper">
					...
					<div class="uiv2-shopping-list-left-column">
						...
						<div class="uiv2-facet-categories">
							...
							<ul>
									...
									%>
										...
										<li><a href="GetListFromSideBar.action?startIndex=1&categoryName=<%=sibeBarArr[1].replaceAll("&", ",,")%>&rootCategory=<%=rootCategory%>&subRootCategory=<%=subCatId%>" class="active"><label>&gt;</label>&nbsp;&nbsp;<B><%=sibeBarArr[1] %></B> <span class="count"></span></a>
											...
											<ul>
												...
												<li><a href="GetListFromSideBar.action?startIndex=1&categoryName=<%=sideBarProductsArr[2].replaceAll("&", ",,")%>&rootCategory=<%=rootCategory%>&subRootCategory=<%=subCatId%>&categoryId=<%=subChldId%>"><label>&gt;</label>&nbsp;&nbsp;<%=sideBarProductsArr[2] %> <span class="count">(<%=sideBarProductsArr[1] %>)</span></a></li>
						...
						<div class="uiv2-price-listing">
							...
							<input type="hidden" name="searchStartIndex" value="1">
					...
					int displaySize = 12;
					String startIndexVal = request.getParameter("startIndex") != null ? request.getParameter("startIndex") : (String)request.getAttribute("startIndexWishList");
					//int startIndex = (Integer.parseInt(startIndexVal)-1) * displaySize;

					/* //int startIndex=1;

					int endIndex = startIndex*5+displaySize;
					...
					int selectedIndex = Integer.parseInt(request.getAttribute("startIndex").toString().trim());
					...
					<input type="hidden" name="rootCategory" id="rootCategory" value="<%=rootCategory%>"/>

					<input type="hidden" name="rootCategory" id="subRootCategory" value="<%=subRootCategory%>"/>

					<input type="hidden" name="childCategory" id="childCategory" value="<%=categoryId%>"/>

					<input type="hidden" name="categoryName" id="categoryName" value="<%=categoryName.replaceAll("&", ",,")%>">

					<input type="hidden" name="type" id="type" value="productsForCategory">
					<input type="hidden" name="startIndex" id="startIndex" value="<%=startIndexVal %>">
					...
					<div class="uiv2-shopping-list-right-column">
						...
						<div class="uiv2-title-breads-wrap">
							...
							<%=request.getAttribute("categoryName") != null ? request.getAttribute("categoryName").toString().replaceAll(",,", "&") : "" %>
							 ...
							 (<span id="uiv2-num-products"><s:property value="productCount"/></span>)</h4>

						</div>

						<div class="paginationArea">

							<div class="noItems"> Showing <span class="bFont"><%=startIndexShow %> - <%=endIndex %></span> of <span class="bFont"><%=count %> products</span></div>
								...
								<ul class="pages">
									<%
										for(int x = 1 ; x<=pageNum; x++){
											if(x==selectedIndex){
									%>
									<li><a href="ViewProductsForcategory.action?categoryName=<%=categoryName%>&startIndex=<%=x %>&categoryId=<%=categoryId %>&subRootCategory=<%=subRootCategory%>&rootCategory=<%=rootCategory%>" class="selected"><%=x %></a></li>
									<%
										}
										else{
											%>
										<li><a href="ViewProductsForcategory.action?categoryName=<%=categoryName%>&startIndex=<%=x %>&categoryId=<%=categoryId%>&subRootCategory=<%=subRootCategory%>&rootCategory=<%=rootCategory%>"><%=x %></a></li>
										<%	
										}
										}
									%>
						...
						<div class="uiv2-div100" id="facet-products-wrapper">
							...
							<div id="facet-loading-wrap">
								...
								<div id="facet-loading"><img src="images/loading_brown.gif" /><span>Loading...</span></div>
									...
									<%-- <s:iterator value="productList" id="product">
									 ...
									 --%>	<%
											...
											/* for(int index = startIndex;index<endIndex;index++){ */
												...
												double saveValue = Double.parseDouble(((String)al.get(3)).trim())-Double.parseDouble(((String)al.get(4)).trim());
												...
												ArrayList<String[]> priceDetail = (ArrayList<String[]>)al.get(6);
												...
												for(int cookieIndex = 0; cookieIndex<c.length; cookieIndex++){
													...
													if(al.get(0).equals(cookieName)){
														...
														String allValues[] = (c[cookieIndex].getValue()).split("-");
										...
										<s:property value="#pInfo[0]"/>
										 ...
										 <li>
											...
											<div class="uiv2-combo-block">
												...
												<span>SAVE <span class="WebRupee">Rs.</span><span id='<%="saveValueShow" + al.get(0)%>'> 
												 <%=saveValue %></span></span>
											...
											<div class="uiv2-list-box-img-block">
												...
												<a href="ProductsDetailForCategory.action?productId=<%=al.get(0)%>" title=""><img title="" alt="" width="150" height="200" src="<%=imagePath %>" /></a>
											...
											<div class="uiv2-list-box-img-title">
												...
												<span class="uiv2-title-tool-tip">
													...
													<a href="javascript:void();"><span class="uiv2-brand-title">

													<input type="hidden" name="" value="<%=al.get(2) %>" id='<%="brandName"+al.get(0)%>'/>

													<input type="hidden" name="" value="<%=al.get(1) %>" id='<%="productName"+al.get(0)%>'/>

													<input type="hidden" name="" value="<%=imagePath %>" id='<%="imagePath"+al.get(0)%>'/>

													<input type="hidden" name="" value="<%=saveValue %>" id='<%="saveValue"+al.get(0)%>'/>

													<%=al.get(2) %></span> <%=al.get(1) %> </a>

													<span class="uiv2-tool-tip-hover "><%=al.get(2) %> &nbsp;&nbsp; <%=al.get(1) %></span></span>
											...
											<div class="uiv2-list-box-drop-rate-block">
												...
												<div class="ListBox_Rate_count">
													...
													<div class="Rate_count_low">MRP: <span><span class="WebRupee">Rs.</span>
													<span id='<%="actualPriceShow" + al.get(0)%>' ><%=al.get(3) %></span></span></div>

													<div class="uiv2-rate-count-avial"><span class="WebRupee">Rs.</span>
													<input type="hidden" id='<%="offerPrice"+al.get(0)%>' value="<%=al.get(4)%>" name='<%="offerPrice"+al.get(0)%>'/> 
												...
												<div class="uiv2-rate-button-block">
													...
													<div class="uiv2-add-to-basket">
														...
														<div class="uiv2-rate-count-btn"><span class="uiv2-qty-label">Qty</span>
															...
															<input value="<%=alreadyInBasket %>" type="hidden" name='<%="quantity"+al.get(0)%>' id='<%="quantity"+al.get(0)%>' size="10">

															<input value="<%=alreadyInBasketView %>" type="text" name='<%="quantityShow"+al.get(0)%>' id='<%="quantityShow"+al.get(0)%>' onBlur="quantityAdd('<%=al.get(0) %>')" size="10">

														</div>

														<a class="uiv2-add-button a2c" onClick="javascript:addToCart('<%=al.get(0) %>')" href="javascript:void(0)"> ADD<span class="icon icon-basket-gray"></span></a>

													</div>

												</div>
												<div class="uiv2-rate-button-block"><a href="javascript:void();" onclick="addToWishList('<%=al.get(0) %>')" class="wishlist">add to wishlist</a></div>
											</div> 
										</li>
								...
								<ul class="pages">
									<%
										for(int y = 1 ; y<=pageNum; y++){
											if(y==selectedIndex){
									%>
									<li><a href="ViewProductsForcategory.action?categoryName=<%=categoryName%>&startIndex=<%=y%>&categoryId=<%=categoryId%>&subRootCategory=<%=subRootCategory%>&rootCategory=<%=rootCategory%>" class="selected"><%=y %></a></li>									<%
										}
										else{
											%>
										<li><a href="ViewProductsForcategory.action?categoryName=<%=categoryName%>&startIndex=<%=y%>&categoryId=<%=categoryId%>&subRootCategory=<%=subRootCategory%>&rootCategory=<%=rootCategory%>"><%=y %></a></li>
										<%	
										}
										}
									%>
