...
public class MassUpdateProduct implements ServletResponseAware,SessionAware, ServletRequestAware{
private static final Logger logger = Logger.getLogger(DownloadAllProductAction.class.getName());
	...
	public String getMassUpdateProductForm(){
	...
	public String downloadProductForUpdate() throws IOException{
	
		XSSFWorkbook workbook = null;
        Connection connection = null;
		try{
			
			String rootCategory = request.getParameter("categoryId");
			String subRootCategory = request.getParameter("subCategoryId");
			String childCategory = request.getParameter("childCategoryName");
			
			Map parameter = new HashMap();
			parameter.put("rootcategory", rootCategory);
			parameter.put("subrootcategory", subRootCategory);
			parameter.put("childcategory", childCategory);
			
			connection = DBConnection.getConnection();
			
	        IMassUploadService objMassUploadService = new MassUploadServiceImpl();
	        workbook = objMassUploadService.downloadProductForUpdate(connection, parameter);
		}
		catch (Exception e) {
			logger.error(e);
		}
	...
	public String execute(){
		...
		try{
			...
			for(int i = 1; i < rows; i++) { 
				...
				String newActualPrice = cellToString(row.getCell(7));
				boolean newActualPriceFlag = true;
				try{
					double d1 = Double.parseDouble(newActualPrice);
				}
				catch (Exception e) {
					newActualPriceFlag = false;
					actionFlag = false;
					//newActualPrice = "0.0";
				}
				
				String offerPrice = cellToString(row.getCell(8));
				...
				String newOfferPrice = cellToString(row.getCell(9));
				boolean newOfferPriceFlag = true;
				try{
					double d = Double.parseDouble(newOfferPrice);
				}
				catch (Exception e) {
					actionFlag = false;
					newOfferPriceFlag = false;
					//newOfferPrice = "0.0";
				}
				
				String size = cellToString(row.getCell(10));
				String aboutProduct = cellToString(row.getCell(11));
				String ingredient = cellToString(row.getCell(12));
				String imagePath = cellToString(row.getCell(13));
				String action = cellToString(row.getCell(14));
				...
				parameters.put("newactualprice", newActualPrice);
				parameters.put("offerprice", offerPrice);
				parameters.put("newofferprice", newOfferPrice);
				...
				if(actionFlag){
					try{
						...
						msg = "Product has not been added due to some error."+e.getMessage();
						logger.error("",e);
					}
				...
				Cell newActualPriceCell = rowCreate.createCell(7);
				if(!newActualPriceFlag){
					newActualPriceCell.setCellValue(newActualPrice);
					
					ClientAnchor anchorActPrice = factory.createClientAnchor();
					anchorActPrice.setCol1(newActualPriceCell.getColumnIndex());
					anchorActPrice.setCol2(newActualPriceCell.getColumnIndex()+3);
					anchorActPrice.setRow1(rowCreate.getRowNum());
					anchorActPrice.setRow2(rowCreate.getRowNum()+2);
					
					Comment commentActPrice = drawing.createCellComment(anchorActPrice);
				    RichTextString strActPrice = factory.createRichTextString("New Actual Price is not Valid.");
				    commentActPrice.setString(strActPrice);
				    commentActPrice.setAuthor("BestKart4U.com");
				    newActualPriceCell.setCellComment(commentActPrice);
				}
				else{
					newActualPriceCell.setCellValue(Double.parseDouble(newActualPrice));
				}
				newActualPriceCell.setCellStyle(style);
				
				Cell offerPriceCell = rowCreate.createCell(8);
				if(!offerPriceFlag){
					offerPriceCell.setCellValue(offerPrice);
					
					ClientAnchor anchorOfferPrice = factory.createClientAnchor();
					anchorOfferPrice.setCol1(offerPriceCell.getColumnIndex());
					anchorOfferPrice.setCol2(offerPriceCell.getColumnIndex()+3);
				...
				Cell newOfferPriceCell = rowCreate.createCell(9);
				if(!offerPriceFlag){
					newOfferPriceCell.setCellValue(newOfferPrice);
					
					ClientAnchor anchorOfferPrice = factory.createClientAnchor();
					anchorOfferPrice.setCol1(newOfferPriceCell.getColumnIndex());
					anchorOfferPrice.setCol2(newOfferPriceCell.getColumnIndex()+3);
					anchorOfferPrice.setRow1(rowCreate.getRowNum());
					anchorOfferPrice.setRow2(rowCreate.getRowNum()+2);
					
					Comment commentOfferPrice = drawing.createCellComment(anchorOfferPrice);
				    RichTextString strOfferPrice = factory.createRichTextString("New Offer Price is not Valid.");
				    commentOfferPrice.setString(strOfferPrice);
				    commentOfferPrice.setAuthor("BestKart4U.com");
				    newOfferPriceCell.setCellComment(commentOfferPrice);
				}
				else{
					newOfferPriceCell.setCellValue(Double.parseDouble(newOfferPrice));
				}
				newOfferPriceCell.setCellStyle(style);
				
				Cell cell6 = rowCreate.createCell(10);
				cell6.setCellValue(size);
				cell6.setCellStyle(style);
				
				Cell cell7 = rowCreate.createCell(11);
				cell7.setCellValue(aboutProduct);
				cell7.setCellStyle(style);
				
				Cell cell8 = rowCreate.createCell(12);
				cell8.setCellValue(ingredient);
				cell8.setCellStyle(style);
				
				Cell cell9 = rowCreate.createCell(13);
				cell9.setCellValue(imagePath);
				cell9.setCellStyle(style);
				
				Cell actionCell = rowCreate.createCell(14);
			...
			//createHelper.
			rowSummary.createCell(13).setCellValue(createHelper.createRichTextString("Number of success Record : "));
			rowSummary.createCell(14).setCellValue(successCount);
			
			//rowSummary.setHeight((short)1000); 
			
			Row rowSummary1 = sheetCreate.createRow((int)(successCount + failCount + 3));
			
			rowSummary1.createCell(13).setCellValue("Number of fail Record : ");
			rowSummary1.createCell(14).setCellValue(failCount);
